...
from excel_writers.colour_function import colour_cells
from excel_writers.styling_dicts import colour_dict, highlight_dict

TODAY = datetime.today().strftime("%d.%m.%Y")


class ExcelWriter:
    def __init__(self, excel_file_name, dataframe_dict):
        ...
        if len(list(dataframe_dict.items())[0][1]):
            ...
            try:
                excel_writer = pd.ExcelWriter(f"{TODAY}/{excel_file_name}.xlsx", engine="openpyxl", mode="a")
            except FileNotFoundError:
                excel_writer = pd.ExcelWriter(f"{TODAY}/{excel_file_name}.xlsx", engine="openpyxl", mode="w")
